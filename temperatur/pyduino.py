...
class Arduino():
    ...
    def sensor_read(self, pin_number):
        """
        Performs a DHT22 read on pin_number and returns the value for temperatur and humidity
        Internally sends b'RD{pin_number}' over the serial connection
        """
        command = (''.join(('RS', str(pin_number)))).encode()
        self.conn.write(command)
        line_received = self.conn.readline().decode().strip()
        header_temp, value_temp, header_hum, value_hum,  = line_received.split(':')  # e.g. D13:1
        # return line_received
        return {'temp':value_temp,'hum':value_hum}
